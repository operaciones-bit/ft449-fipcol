...
def generar_pdf_formato_informativo(datos):
    """Genera el Formato Informativo de Cliente con los datos de la consulta."""
    from reportlab.lib.pagesizes import letter
    from reportlab.pdfgen import canvas
    from pypdf import PdfReader, PdfWriter

    pdf_base_path = os.path.join(os.path.dirname(__file__), 'Formato_Informativo_base.pdf')
    ...
    # Crear una capa de texto con ReportLab encima del PDF
    from reportlab.lib.pagesizes import letter
    from reportlab.pdfgen import canvas
    import io

    def texto_sobre_pagina(ancho, alto, campos_pagina):
        buf = io.BytesIO()
        c = canvas.Canvas(buf, pagesize=(ancho, alto))
        c.setFont("Helvetica", 8)
        c.setFillColorRGB(0, 0, 0.5)  # azul oscuro
        for (x, y, texto) in campos_pagina:
            if texto:
                c.drawString(x, y, str(texto)[:60])
        c.save()
        buf.seek(0)
        return buf
    ...
    cedula     = limpiar(datos.get('cedula', ''))
    telefono   = limpiar(datos.get('telefono', ''))
    correo     = limpiar(datos.get('correo', ''))
    pagaduria  = limpiar(datos.get('pagaduria', ''))
    entidad    = limpiar(datos.get('entidad', ''))
    monto      = limpiar(datos.get('monto', ''))
    plazo      = limpiar(datos.get('plazo', ''))
    tasa       = limpiar(datos.get('tasa', ''))
    descuento  = limpiar(datos.get('descuento', ''))
    direccion  = limpiar(datos.get('direccion', ''))
    ciudad     = limpiar(datos.get('ciudad', ''))
    departamento = limpiar(datos.get('departamento', ''))
    estrato    = limpiar(datos.get('estrato', ''))
    vivienda   = limpiar(datos.get('tipoVivienda', ''))
    fecha_nac  = limpiar(datos.get('fechaNacimiento', ''))
    edad       = limpiar(datos.get('edad', ''))
    genero     = limpiar(datos.get('genero', ''))
    estado_civil = limpiar(datos.get('estadoCivil', ''))
    estudios   = limpiar(datos.get('estudios', ''))
    estatura   = limpiar(datos.get('estatura', ''))
    peso       = limpiar(datos.get('peso', ''))
    personas_cargo = limpiar(datos.get('personasCargo', ''))
    tipo_ocupacion = limpiar(datos.get('tipoOcupacion', ''))
    tipo_contrato  = limpiar(datos.get('tipoContrato', ''))
    empresa    = limpiar(datos.get('empresa', ''))
    cargo      = limpiar(datos.get('cargo', ''))
    ingresos   = limpiar(datos.get('ingresos', ''))
    egresos    = limpiar(datos.get('egresos', ''))
    banco      = limpiar(datos.get('banco', ''))
    tipo_cuenta = limpiar(datos.get('tipoCuenta', ''))
    num_cuenta = limpiar(datos.get('numeroCuenta', ''))
    ref1_nombre = limpiar(datos.get('ref1Nombre', ''))
    ref1_cel   = limpiar(datos.get('ref1Celular', ''))
    ref1_relacion = limpiar(datos.get('ref1Relacion', ''))
    ref2_nombre = limpiar(datos.get('ref2Nombre', ''))
    ref2_cel   = limpiar(datos.get('ref2Celular', ''))
    ref2_parentesco = limpiar(datos.get('ref2Parentesco', ''))
    asesor     = limpiar(datos.get('asesor', ''))
    id_consulta = limpiar(datos.get('idConsulta', ''))
    fecha_hoy  = limpiar(datos.get('fechaHoy', ''))

    # Separar apellidos y nombres
    partes = nombre.upper().split()
    if len(partes) >= 4:
        apellidos = ' '.join(partes[:2]); nombres_p = ' '.join(partes[2:])
    elif len(partes) == 3:
        apellidos = partes[0]; nombres_p = ' '.join(partes[1:])
    elif len(partes) == 2:
        apellidos = partes[0]; nombres_p = partes[1]
    else:
        apellidos = nombre.upper(); nombres_p = ''

    # ── COORDENADAS PÁGINA 1 ─────────────────────────────────────────
    # (x, y, valor)  — y=0 es la parte de abajo de la página
    P1 = letter[1]  # alto de página en puntos (~792)

    campos_p1 = [
        # Encabezado
        (390, P1 - 62,  fecha_hoy),
        (390, P1 - 72,  id_consulta),
        (390, P1 - 82,  asesor),
        # Sección 1 — Datos básicos
        (45,  P1 - 138, apellidos),
        (310, P1 - 138, nombres_p),
        (45,  P1 - 165, cedula),
        (310, P1 - 165, limpiar(datos.get('fechaExpedicion', ''))),
        (45,  P1 - 192, limpiar(datos.get('lugarExpedicion', ''))),
        (310, P1 - 192, fecha_nac),
        (45,  P1 - 219, limpiar(datos.get('lugarNacimiento', ''))),
        (310, P1 - 219, edad),
        (45,  P1 - 253, genero),
        (215, P1 - 253, estado_civil),
        (390, P1 - 253, estudios),
        (45,  P1 - 280, estatura),
        (215, P1 - 280, peso),
        (390, P1 - 280, personas_cargo),
        # Sección 2 — Dirección
        (45,  P1 - 322, direccion),
        (45,  P1 - 356, ciudad),
        (215, P1 - 356, departamento),
        (390, P1 - 356, limpiar(datos.get('tiempoResidencia', ''))),
        (45,  P1 - 383, vivienda),
        (215, P1 - 383, estrato),
        (390, P1 - 383, telefono),
        (45,  P1 - 410, correo),
        (215, P1 - 410, limpiar(datos.get('telAlterno', ''))),
        # Sección 3 — Solicitud
        (45,  P1 - 453, entidad),
        (310, P1 - 453, pagaduria),
        (45,  P1 - 480, limpiar(datos.get('tipoSolicitud', ''))),
        (310, P1 - 480, limpiar(datos.get('tipoCredito', ''))),
        (45,  P1 - 507, monto),
        (310, P1 - 507, plazo),
        (45,  P1 - 534, tasa),
        (310, P1 - 534, descuento),
    ]

    # ── COORDENADAS PÁGINA 2 ─────────────────────────────────────────
    P2 = letter[1]

    campos_p2 = [
        # Sección 4 — Laboral
        (45,  P2 - 55,  tipo_ocupacion),
        (310, P2 - 55,  tipo_contrato),
        (45,  P2 - 82,  empresa),
        (310, P2 - 82,  cargo),
        (45,  P2 - 109, limpiar(datos.get('dirEmpresa', ''))),
        (310, P2 - 109, limpiar(datos.get('ciudadEmpresa', ''))),
        (45,  P2 - 136, limpiar(datos.get('telEmpresa', ''))),
        (310, P2 - 136, limpiar(datos.get('fechaIngreso', ''))),
        (45,  P2 - 163, limpiar(datos.get('area', ''))),
        (310, P2 - 163, limpiar(datos.get('correoLaboral', ''))),
        (45,  P2 - 190, limpiar(datos.get('jefe', ''))),
        (310, P2 - 190, limpiar(datos.get('cargoJefe', ''))),
        # Sección 5 — Financiera
        (45,  P2 - 233, ingresos),
        (215, P2 - 233, limpiar(datos.get('otrosIngresos', ''))),
        (390, P2 - 233, limpiar(datos.get('origenIngresos', ''))),
        (45,  P2 - 260, egresos),
        (215, P2 - 260, limpiar(datos.get('activos', ''))),
        (390, P2 - 260, limpiar(datos.get('pasivos', ''))),
        (45,  P2 - 287, limpiar(datos.get('capacidadPago', ''))),
        (215, P2 - 287, limpiar(datos.get('cuotaEstimada', ''))),
        (390, P2 - 287, limpiar(datos.get('origenFondos', ''))),
        (45,  P2 - 314, limpiar(datos.get('usuarioPlat', ''))),
        (310, P2 - 314, limpiar(datos.get('clavePlat', ''))),
        # Sección 6 — Desembolso
        (45,  P2 - 357, banco),
        (215, P2 - 357, tipo_cuenta),
        (390, P2 - 357, num_cuenta),
        # Sección 7 — Cónyuge
        (45,  P2 - 400, limpiar(datos.get('conyugeNombre', ''))),
        (310, P2 - 400, limpiar(datos.get('conyugeCedula', ''))),
        (45,  P2 - 427, limpiar(datos.get('conyugeEmpresa', ''))),
        (310, P2 - 427, limpiar(datos.get('conyugeCargo', ''))),
        (45,  P2 - 454, limpiar(datos.get('conyugeTel', ''))),
        (310, P2 - 454, limpiar(datos.get('conyugeCel', ''))),
        # Sección 8 — Ref personal
        (45,  P2 - 497, ref1_nombre),
        (215, P2 - 497, ref1_relacion),
        (390, P2 - 497, ref1_cel),
        (45,  P2 - 524, limpiar(datos.get('ref1Ciudad', ''))),
        (215, P2 - 524, limpiar(datos.get('ref1Empresa', ''))),
        (390, P2 - 524, limpiar(datos.get('ref1Tel', ''))),
        # Sección 9 — Ref familiar
        (45,  P2 - 567, ref2_nombre),
        (215, P2 - 567, ref2_parentesco),
        (390, P2 - 567, ref2_cel),
        (45,  P2 - 594, limpiar(datos.get('ref2Direccion', ''))),
        (215, P2 - 594, limpiar(datos.get('ref2Ciudad', ''))),
        (390, P2 - 594, limpiar(datos.get('ref2TelRes', ''))),
        (45,  P2 - 621, limpiar(datos.get('ref2Empresa', ''))),
        (215, P2 - 621, limpiar(datos.get('ref2TelTrab', ''))),
    ]

    # Superponer texto sobre cada página
    ...
    # Página 1
    overlay_p1 = PR2(texto_sobre_pagina(pw, ph, campos_p1)).pages[0]
    writer.pages[0].merge_page(overlay_p1)

    # Página 2
    overlay_p2 = PR2(texto_sobre_pagina(pw, ph, campos_p2)).pages[0]
    writer.pages[1].merge_page(overlay_p2)
